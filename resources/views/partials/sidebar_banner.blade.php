            ...
            <div class="accordion-item rounded-0" style="border: none;">
                <h2 class="accoridon-header mb-0" id="headplatform-{{ $i }}">
                    <button class="accordion-button rounded-0 collapsed bg-white" type="button"
                        data-mdb-toggle="collapse" data-mdb-target="#colplatform-{{ $i }}"
                        aria-expanded="false" aria-controls="colplatform-{{ $i }}"
                        value="{{ $i }}">
                        @if ($i == 1)
                ...
                <div id="colplatform-{{ $i }}"
                    class="accordion-collapse collapse collapse-horizontal platform"
                            ...
                            <div class="accordion-item rounded-0 border-0 mb-2">
                                <h2 class="accordion-header" id="heading{{ $pos->id_slot }}">
                                    <button class="accordion-button rounded-0 collapsed" type="button"
                                        data-mdb-toggle="collapse" data-mdb-target="#collapse{{ $pos->id_slot }}"
                                        aria-expanded="true" aria-controls="collapse{{ $pos->id_slot }}"
                                        value="{{ $pos->id_slot }}" name="banneritem">
                                        {{ $pos->slot_name }}
                                    </button>
                                </h2>
                                        ...
                                        <ul class="nav nav-tabs" role="tablist">
                                            @foreach ($joinbanner->where('id_slot', $pos->id_slot) as $banner)
                                                <li class="nav-item w-100" role="presentation">
                                                    <p class="nav-link b-select" id="bnr-{{ $banner->id }}"
